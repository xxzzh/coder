...
def optional_extractors() -> dict[str, object]:
    ...
    return {
        ...
        "ocr_ready": ocr_ready,
        "ocr_offline_ready": ocr_ready,
        "ocr_online_ready": ocr_ready,
    }
...
def api_configured(env: dict[str, str]) -> bool:
    return (
...
def online_translation_status(env: dict[str, str]) -> dict[str, object]:
    ready = api_configured(env) and api_backend_allowed(env)
    return {
        "en_to_zh_ready": ready,
